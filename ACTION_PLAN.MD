...
-   💳 Multi-card payment method tracking with credit utilization
-   🤝 Shared expense management with custom splits
-   📊 Real-time budget tracking with visual indicators
-   💵 Income management (fixed + variable)
...
This project follows a **design-first, understanding-always** approach. The goal is to build real engineering judgment — knowing how to design systems, review code critically, and make architectural decisions. AI handles implementation of known patterns; the user writes code when encountering new concepts.

### Two Modes of Working

**Mode 1 (Default): User Implements**
- User writes the code, even for known patterns — repetition builds muscle memory
- AI mentors: answers questions, gives hints when stuck, explains concepts
- AI reviews after: critiques code, suggests improvements, catches issues
- Used for: all features, services, controllers, routes, business logic, tests

**Mode 2: AI Implements (Boilerplate Only)**
- AI writes code only for genuinely repetitive, zero-learning-value tasks
- Config files, mechanical refactors, import wiring, repetitive scaffolding
- The test: if writing it teaches something, user writes it (Mode 1)

**Mode 3: Deep Learning (New Concepts)**
- AI flags when a new pattern/concept appears
- AI explains the concept first, then user writes the code
- AI reviews and provides feedback after

### The Check

Before accepting AI-generated code, ask yourself:
- Can I explain what this code does and why?
- What would break if I changed a key part?
- Could I explain this to a colleague?
- If this fails in production, where would I look?
...
## 🎓 Learning Philosophy & AI Guidelines

### **Core Principles**

1. **Design first** — Understand the approach before code gets written
2. **Understand always** — Never accept code you can't explain
3. **New concepts = hands-on** — Write it yourself the first time
4. **Known patterns = AI implements** — Don't re-type what you already understand
5. **Review critically** — Read AI code like you'd review a teammate's PR

### **AI Working Rules** ⚠️ CRITICAL

#### **AI SHOULD:**

-   ✅ Flag new concepts and ask if user wants to write it
-   ✅ Implement known patterns following codebase conventions
-   ✅ Explain what was written and key decisions
-   ✅ Discuss design/trade-offs before implementing
-   ✅ Flag security vulnerabilities immediately
-   ✅ Run tests and linter after changes
-   ✅ Update ACTION_PLAN.MD after completing sections

#### **AI SHOULD NOT:**

-   ❌ Skip design discussion before significant code
-   ❌ Implement new concepts without flagging them
-   ❌ Make architectural decisions silently
-   ❌ Generate code the user can't explain
...
-   ✅ Week 4: Backend foundation (auth working)
-   ✅ Week 9: Core features complete (testable with Postman)
-   ✅ Week 16: AI features working
-   ✅ Week 25: Full app usable (replace Excel)
-   ✅ Week 31: Production deployed
-   ✅ Week 34: Polished with OAuth & i18n
...
## Global PR Strategy (applies to all phases below)

From Phase 2 onwards, every sub-phase is structured as **vertical slices** — small, independently shippable PRs marked with `[PR]`.

**Goal**: ship one PR every 1-2 days. Each slice should be:
-   Reviewable in under 30 minutes
-   Ends with passing tests + green CI
-   Not bundled with unrelated work

**Default split pattern per feature domain** (budgets, income, debt, etc.):
...
4.  Manual E2E testing in Postman (no PR — just the verification step)

When in doubt, split smaller. It's better to ship 3 small PRs than 1 large one.
...
### **PR Strategy (Updated April 2026)**

**Goal**: Ship one PR every 1-2 days. Sub-phases are split into **vertical slices**, each independently shippable. Prefer small, focused PRs over large bundled ones.

A vertical slice = a self-contained piece of work that can be reviewed, tested, and merged on its own. For example, "Service + types + unit tests" is one slice; "Controller + routes + validation + integration tests" is another.
...
#### 2.1.1: Create Expense Service ✅ (PR #11 — merged)
...
-   [x] Unit tests (29 tests covering all service methods)

#### 2.1.2: Create Expense Controller ✅

-   [x] Create controllers/expense.controller.ts
-   [x] Implement createExpense handler
-   [x] Implement getAllExpenses handler
-   [x] Implement getExpenseById handler
-   [x] Implement updateExpense handler
-   [x] Implement deleteExpense handler

#### 2.1.3: Create Expense Routes ✅

-   [x] Create routes/expense.routes.ts
-   [x] Define POST /api/expenses
-   [x] Define GET /api/expenses
-   [x] Define GET /api/expenses/:id
-   [x] Define PATCH /api/expenses/:id
-   [x] Define DELETE /api/expenses/:id
...
-   [x] Define createExpenseSchema (Zod)
-   [x] Define updateExpenseSchema
-   [x] Define getExpensesQuerySchema (filters)
-   [x] Define expenseIdParamSchema (route params)
-   [x] Extend validateRequest middleware to handle Body / Params / Query

#### 2.1.5: Manual Testing (Postman) ✅

-   [x] Create expense via POST
-   [x] Verify actualExpenditure calculated
-   [x] Get all expenses
-   [x] Get single expense
-   [x] Update expense
-   [x] Delete expense
-   [x] Test with invalid category UUID
-   [x] Test ownership (another user's expense)

#### 2.1.6: Controller Unit Tests ✅

-   [x] Add expense.controller.test.ts
-   [x] Test all 5 handler methods (mock service layer)
-   [x] Test success responses (status code, response shape)
-   [x] Test error propagation to next(error) — parameterized with `it.each`
...
Split into PR-sized vertical slices. Each `[PR]` marker indicates a separate, mergeable PR.
...
-   [ ] Create controllers/category.controller.ts (all handlers)
...
Split into vertical slices. Each `[PR]` marker = a mergeable PR.
...
**Phase 2 Status**: 🔒 Locked → 🟡 In Progress → 🟢 Completed
...
**Phase 3 Status**: 🔒 Locked → 🟡 In Progress → 🟢 Completed
...
**Phase 4 Status**: 🔒 Locked → 🟡 In Progress → 🟢 Completed
...
**Phase 5 Status**: 🔒 Locked → 🟡 In Progress → 🟢 Completed
...
> Note: Frontend slices below are mostly self-contained pages/features. Each `[PR]` is independently shippable. Bundle together only if a feature is truly trivial.
...
#### 6.1.5: TanStack Query Configuration

> Folded into 6.1.4 above (API client + TanStack are tightly coupled).
...
**Phase 6 Status**: 🔒 Locked → 🟡 In Progress → 🟢 Completed
...
> Note: If you've been writing tests as you build (per Phase 2-6 strategy), most of this is already done. This sub-phase becomes a "fill the gaps" pass.
...
**Phase 7 Status**: 🔒 Locked → 🟡 In Progress → 🟢 Completed
...
**Phase 8 Status**: 🔒 Locked → 🟡 In Progress → 🟢 Completed
...
**Phase 9 Status**: 🔒 Locked → 🟡 In Progress → 🟢 Completed
...
✅ Secure authentication with JWT, OAuth, and 2FA  
✅ Comprehensive financial features (budgets, debts, goals)  
...
**Plan Version**: 1.2  
**Last Updated**: April 2026 (PR strategy applied across all phases)  
**Estimated Total Time**: 8-9 months (5-7 hours/week)  
**Maintainer**: You + AI Assistant (mentor/reviewer role)

---

_"The best way to learn is by doing. Build something real, make mistakes, learn from them, and iterate. You've got this!"_ 🚀
